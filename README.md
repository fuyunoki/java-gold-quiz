...
## 収録済み分野（学習パス順・計 170 問）

| 順 | 分野 | 問題数 |
|----|------|--------|
| 1 | ラムダ式と関数型インターフェース | 15 |
| 2 | Stream API | 15 |
| 3 | ジェネリクス | 15 |
| 4 | コレクション | 15 |
| 5 | 例外処理とアサーション | 14 |
| 6 | モダンJava（record / sealed / switch式） | 15 |
| 7 | 並行処理 | 14 |
| 8 | NIO.2（ファイルAPI） | 14 |
| 9 | モジュールシステム（JPMS） | 14 |
| 10 | JDBC | 13 |
| 11 | ローカライゼーション | 12 |
| 12 | 日付・時刻API（java.time） | 14 |

各問は難易度（基礎／標準／応用）付きで、解説は Java Silver 取得者向けに
「なぜそうなるか」を噛み砕いて記述しています。

> 分野 7〜12 は Java Gold SE17 の出題範囲・形式・難易度に沿って作成した
> **オリジナル問題**です（Oracle の実試験問題そのものの転載ではありません）。
> 今後 アノテーション / java.io・シリアライズ / 内部クラス 等も同形式で追加できます。
